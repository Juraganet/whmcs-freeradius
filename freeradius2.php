...
function freeradius_AdminServicesTabFields($params){
  ...
  $fieldsarray = array(
   ...
   'Accumalated Hours status' => secs_to_h( $collected['logintime'] ),
...
function freeradius_ClientArea($params){
  ...
  $query = "SELECT value from radcheck where username='$username' AND attribute='Cleartext-Password'";
...
function freeradius_CreateAccount($params){
  ...
  //Insert into daloradius user
  //insert userinfo
  $currDate = date('Y-m-d H:i:s');
  $currBy = "WHMCS";

  	// insert user information table
  	$query = "INSERT INTO userinfo (id, username, firstname, lastname, email, workphone, creationby) ".
			" VALUES (0,'$username','$firstname','$lastname','$email','$phonenumber','$currBy')";
							
	$res = mysql_query($query,$freeradiussql);
	if( !$res ){
	    $radiussqlerror = mysql_error();
	    freeradius_WHMCSReconnect();
	    return "FreeRadius Database Query Error: " . $radiussqlerror;
	...
	// insert user information table
	$query = "INSERT INTO userbillinfo (id, username, contactperson, email, phone, creationby) ".
					" VALUES (0,'$username', '$firstname', '$email', '$phonenumber', '$currBy')";
...
function freeradius_UnsuspendAccount($params){
  ...
  $query = "DELETE FROM radacct WHERE 1=1 AND Username='$username'";
...
function freeradius_TerminateAccount($params){
  ...
  $query = "DELETE FROM userbillinfo WHERE username='$username'";
  $result = mysql_query($query,$freeradiussql);
  if (!$result) {
    $radiussqlerror = mysql_error();
    freeradius_WHMCSReconnect();
